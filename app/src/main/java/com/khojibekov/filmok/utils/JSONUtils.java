...
public static ArrayList<Movie> getMoviesFromJSON(JSONObject jsonObject){
    ...
    try {
        ...
        for (int i = 0; i < jsonArray.length(); i++){
            ...
            double voteAverage = objectMovie.getDouble(KEY_VOTE_COUNT);
            String releaseDate = objectMovie.getString(KEY_RELEASE_DATE);
            Movie movie = new Movie(id, voteCount, title, originalTitle, overview, posterPath, backDropPath, bigPosterPath, voteAverage, releaseDate);
            result.add(movie);
        }
